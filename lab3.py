...
class WebApp(server.App):
    ...
    def getTable(self, params):
        ...
        result_df = result_df[(df["Year"]) >= y1]
        result_df = result_df[(df["Year"]) <= y2]
    ...
    def getPlot(self, params):
        dataframe = self.getTable(params)
        indicator = params["indicator"]
        plt.figure(figsize=(10, 6))

        for y in dataframe["Year"].unique():
            year_data = dataframe[dataframe["Year"] == y]
            plt.plot(year_data["Week"], year_data[indicator], label=f"Year {y}")

        plt.xlabel("Week")
        plt.ylabel(indicator)
        plt.title(f"{indicator} over Weeks for Different Years")
        plt.legend()
        plt.grid(True)
        fig = plt.gcf()
        return fig
